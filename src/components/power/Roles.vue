<template>
  <div>
        ...
        <el-table-column type="expand">
          <template #="{ row }">
            <el-row :class="['bdbottom', index01 === 0 ? 'bdtop' : '', 'vcenter']" v-for="(item01, index01) in row.children" :key="item01.id">
              <!-- 渲染一级权限 -->
              <el-col :span="5"
                ><el-tag closable @close="romoveRightById(row, item01.id)">{{ item01.authName }}</el-tag
              ...
              <el-col :span="19">
                <el-row :class="[index02 === 0 ? '' : 'bdtop', 'vcenter']" v-for="(item02, index02) in item01.children" :key="item02.id">
                  <el-col :span="6"
                    ><el-tag type="success" closable @close="romoveRightById(row, item02.id)">{{ item02.authName }}</el-tag
                    ><i class="el-icon-caret-right"></i
                  ></el-col>
                  <el-col :span="18"
                    ><el-tag type="warning" v-for="item03 in item02.children" :key="item03.id" closable @close="romoveRightById(row, item03.id)">{{ item03.authName }}</el-tag></el-col
                  >
                </el-row>
              </el-col>
            </el-row>
    ...
    <!-- 添加角色对话框 -->
    <el-dialog title="添加角色" v-model="addDialogVisible" width="50%" @close="addFormClose">
    ...
    <!-- 修改角色对话框 -->
    <el-dialog title="修改角色" v-model="modifyDialogVisible" width="50%" @close="modifyFormClose">
      <el-form :model="modifyForm" :rules="modifyRules" ref="modifyFormRef" label-width="70px">
    ...
    <!-- 分配权限对话框 -->
    <el-dialog title="分配权限" v-model="SetRightDialogVisible" width="50%" @close="setRightDialogClose">
  ...
  methods: {
    ...
    modifyRole() {
      this.$refs.modifyFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await this.$http.put('roles/' + this.modifyForm.roleId, this.modifyForm)
